...
class Utility {
	...
	public:
		...
		[[nodiscard]] int getCountedFields() const {
	...
	public:
		bool handlingDuplication(const string& filename, int num_of_fields_in_line) {
			...
			if(bool exists; exists == countCommasInLine(filename, num_of_fields_in_line)) {
				if(counting_fields == num_of_fields_in_line) {
					if(ifstream inFile(filename); !inFile.is_open()) {
						...
						if(num_of_fields_in_line == 1) {
							...
							//issue might be if the first user teacher entered an comma,
	...
	*/
	...
	public:
		static bool renameFileName() {
			...
			const auto OldFile = "temporarly_edit_table.txt";
	...
	public:
		...
		const string& details_of_other_fields, int num_of_fields_on_line) {
			...
			//so another teacher can't register to a course that is existng/tought by a teacher
